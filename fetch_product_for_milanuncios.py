...
def count_pending():
    """Count how many products are still pending publication."""
    payload = {
        "filter": {
            "and": [
                {"property": "In Stock", "checkbox": {"equals": True}},
                {"property": "Milanuncios Posted", "rich_text": {"is_empty": True}}
            ]
        },
        "page_size": 1
    }
    ...
    # Notion doesn't return total count directly, just whether has_more
    # We return has_more as a proxy
    data = resp.json()
...
def main():
    ...
    if not product:
        print('NO_PRODUCTS')
        # Clean up old product_data if exists
        if OUT_FILE.exists():
            OUT_FILE.unlink()
        sys.exit(0)
